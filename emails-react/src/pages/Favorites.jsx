...
const Favorites = () => {
  ...
  const favoriteEmails = emails.filter(email => favoriteIds.includes(email.id));
  ...
  // Agrupar emails favoritos por marca
  const emailsByBrand = {};
  favoriteEmails.forEach(email => {
    if (!emailsByBrand[email.brand]) emailsByBrand[email.brand] = [];
    emailsByBrand[email.brand].push(email);
  });
  Object.keys(emailsByBrand).forEach(brand => {
    emailsByBrand[brand].sort((a, b) => new Date(b.date) - new Date(a.date));
  });

  const filteredEmails = favoriteEmails.filter(email => {
        ...
        <Collapse in={filtersOpen}>
          ...
          {selectedFilterTab === 0 && (
            <Box sx={{ mb: 2 }}>
              ...
              <Collapse in={brandsOpen}>
                <List>
                  {Array.from(new Set(favoriteEmails.map(email => email.brand).filter(Boolean))).map(brand => (
          ...
          {selectedFilterTab === 1 && (
            <Box sx={{ mb: 2 }}>
              ...
              <List>
                {Array.from(new Set(favoriteEmails.map(email => email.flow).filter(Boolean))).map(flow => (
          ...
          {selectedFilterTab === 2 && (
            <Box sx={{ mb: 2 }}>
              ...
              <List>
                {Array.from(new Set(favoriteEmails.map(email => email.campaign).filter(Boolean))).map(campaign => (
